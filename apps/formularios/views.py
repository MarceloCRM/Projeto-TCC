...
def estatisticas_formulario(request, formulario_id):
    ...
    for pergunta in formulario.perguntas.all():
        # Buscamos todas as respostas dessa pergunta específica uma única vez
        respostas_pergunta = Resposta.objects.filter(pergunta=pergunta)
        ...
        if pergunta.tipo == Pergunta.TIPO_ESCOLHA:
            ...
            # Calculamos a porcentagem para cada opção
            for d in dados:
                d['porcentagem'] = (d['total'] / total_respostas * 100) if total_respostas > 0 else 0
            ...
            media = respostas_pergunta.aggregate(media=Avg('valor_numero'))
            estatisticas.append({
                'pergunta': pergunta.texto,
                'tipo': 'numero',
                'media': media['media'],
    ...
    # Calculamos o total de formulários enviados (baseado na primeira pergunta, se houver)
    total_envios = estatisticas[0]['total_geral'] if estatisticas else 0
